...
//
// Lingdong Huang 2020
// Bugfix applied: Corrected thinning loop termination logic.
...
#include <string>
#include <climits>
#include <algorithm> // 添加 algorithm 防止 std::copy 报错
...
// PARAMS
//================================
#define CHUNK_SIZE 35           // the chunk size
...
struct skeleton_tracer_t {
  ...
  void thinning_zs(){
    bool diff;
    do {
      // 修复：必须确保每一步都执行，且只要任意一步改变了像素，就要继续循环
      bool diff0 = thinning_zs_iteration(0);
      bool diff1 = thinning_zs_iteration(1);
      diff = diff0 || diff1; 
    } while (diff);
  }
